...
elif output_format == "vcf" or output_format == "vcf_bi":
    output_string += "##fileformat=VCFv4.0\n"
    output_string += "##fileDate=" + datetime.datetime.today().strftime('%Y%m%d') + "\n"
    output_string += "##source=convert.py\n"
    output_string += "##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype\">\n"
    output_string += "##contig=<ID=scf>\n"
    output_string += "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT"
    for record_id in record_ids:
        output_string += "\t" + record_id    
    output_string += "\n"
    for pos in range(len(record_seqs[0])):
        # First find out what the two alleles are at this pos, for which individuals from all populations must be read.
        alleles_at_this_pos = []
        for x in range(len(record_ids)):
            if record_seqs[x][pos] == "A":
                ...
                alleles_at_this_pos.append("T")
        unique_alleles_at_this_pos = sorted(list(set(alleles_at_this_pos)))
        # Only use biallelic SNPs when vcf_bi is chosen, or all variable sites when vcf
        # is chosen
        use_this_marker = False
        if len(unique_alleles_at_this_pos) == 2:
            use_this_marker = True
            ...
            use_this_marker = True
        if use_this_marker == True:
            line_string = "scf\t" + str(pos+1) + "\t.\t" + unique_alleles_at_this_pos[0] + "\t"
            for unique_allele_at_this_pos in unique_alleles_at_this_pos[1:]:
                line_string += unique_allele_at_this_pos + ","
            line_string = line_string[:-1]
            line_string += "\t.\t.\t.\tGT"
            for x in range(len(record_ids)):
                if record_seqs[x][pos] == "A":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("A")) + "/" + str(unique_alleles_at_this_pos.index("A"))
                elif record_seqs[x][pos] == "C":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("C")) + "/" + str(unique_alleles_at_this_pos.index("C"))
                elif record_seqs[x][pos] == "G":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("G")) + "/" + str(unique_alleles_at_this_pos.index("G"))
                elif record_seqs[x][pos] == "T":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("T")) + "/" + str(unique_alleles_at_this_pos.index("T"))
                elif record_seqs[x][pos] == "M":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("A")) + "/" + str(unique_alleles_at_this_pos.index("C"))
                elif record_seqs[x][pos] == "R":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("A")) + "/" + str(unique_alleles_at_this_pos.index("G"))
                elif record_seqs[x][pos] == "W":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("A")) + "/" + str(unique_alleles_at_this_pos.index("T"))
                elif record_seqs[x][pos] == "S":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("C")) + "/" + str(unique_alleles_at_this_pos.index("G"))
                elif record_seqs[x][pos] == "Y":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("C")) + "/" + str(unique_alleles_at_this_pos.index("T"))
                elif record_seqs[x][pos] == "K":
                    line_string += "\t" + str(unique_alleles_at_this_pos.index("T")) + "/" + str(unique_alleles_at_this_pos.index("T"))
                elif record_seqs[x][pos] == "N":
                    line_string += "\t./."
                elif record_seqs[x][pos] == "-":
                    line_string += "\t./."
                elif record_seqs[x][pos] == "?":
                    ...
                    print("ERROR: Unknown genotype: " + record_seqs[x][pos] + "!")
                    sys.exit(1)
            line_string += "\n"
